...
- (void)initValues
{
    ...
    _topScrollView.pagingEnabled = NO; //分页
    _topScrollView.showsHorizontalScrollIndicator = NO; //水平滚动指示器
    _topScrollView.showsVerticalScrollIndicator = NO;  //垂直滚动指示器
    _topScrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth; //宽度实时自动调整
    [self addSubview:_topScrollView];
    _userSelectedChannelID = 100; //点击按钮选择名字ID
...
- (void)clear
{
    ...
    for (UIView *vc in _rootScrollView.subviews) {
        [vc removeFromSuperview];
    }
    
    for (UIView *vc in _topScrollView.subviews) {
        [vc removeFromSuperview];
    }
}
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [vc.title sizeWithFont:[UIFont systemFontOfSize:kFontSizeOfTabButton]
                               constrainedToSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView)
                                   lineBreakMode:NSLineBreakByTruncatingTail]; //方法已弃用
        //累计每个tab文字的长度
