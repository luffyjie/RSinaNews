        ...
        if (!CGRectContainsPoint([[_viewArr11 objectAtIndex:0] frame], CGPointMake(newX, newY)) ) {
            ...
            if ( _array == _viewArr22) {
                ...
                if ([self buttonInArrayArea1:_viewArr11 Point:point]) {
                    ...
                    int index = ((int)newX - KTableStartPointX)/KButtonWidth + (3 * (((int)newY - KTableStartPointY)/KButtonHeight));
                    ...
                    unsigned long index = ((unsigned long )(newX) - KTableStartPointX)/KButtonWidth + (3 * (((int)(newY) - [self array2StartY] * KButtonHeight - KTableStartPointY)/KButtonHeight));
                    [ _array removeObject:self];
...
- (void)animationAction1{
    for (int i = 0; i < _viewArr11.count; i++) {
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionLayoutSubviews animations:^{
            ...
            [[_viewArr11 objectAtIndex:i] setFrame:CGRectMake(KTableStartPointX + (i%3) * KButtonWidth, KTableStartPointY + (i/3)* KButtonHeight, KButtonWidth, KButtonHeight)];
...
- (void)animationAction2{
    for (int i = 0; i < _viewArr22.count; i++) {
        ...
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionLayoutSubviews animations:^{
            ...
            [[_viewArr22 objectAtIndex:i] setFrame:CGRectMake(KTableStartPointX + (i%3) * KButtonWidth, KTableStartPointY + [self array2StartY] * KButtonHeight + (i/3)* KButtonHeight, KButtonWidth, KButtonHeight)];
...
- (BOOL)buttonInArrayArea1:(NSMutableArray *)arr Point:(CGPoint)point{
    ...
    CGFloat newY = point.y - _point.y + KButtonHeight/2;
    int a =  arr.count%3;
    unsigned long b =  arr.count/3;
    if ((newX > KTableStartPointX && newX < KTableStartPointX + 3 * KButtonWidth && newY > KTableStartPointY && newY < KTableStartPointY + b * KButtonHeight) || (newX > KTableStartPointX && newX < KTableStartPointX + a * KButtonWidth && newY > KTableStartPointY + b * KButtonHeight && newY < KTableStartPointY + (b+1) * KButtonHeight) ) {
        return YES;
    }
    return NO;
}
- (BOOL)buttonInArrayArea2:(NSMutableArray *)arr Point:(CGPoint)point{
    ...
    int a =  arr.count%3;
    unsigned long b =  arr.count/3;
    if ((newX > KTableStartPointX && newX < KTableStartPointX + 3 * KButtonWidth && newY > KTableStartPointY + [self array2StartY] * KButtonHeight && newY < KTableStartPointY + (b + [self array2StartY]) * KButtonHeight) || (newX > KTableStartPointX && newX < KTableStartPointX + a * KButtonWidth && newY > KTableStartPointY + (b + [self array2StartY]) * KButtonHeight && newY < KTableStartPointY + (b+[self array2StartY]+1) * KButtonHeight) ) {
        return YES;
    }
    return NO;
}
- (unsigned long)array2StartY{
    ...
    y = _viewArr11.count/3 + 2;
    if (_viewArr11.count%3 == 0) {
        y -= 1;
    }
    return y;
}
