...
- (void)viewDidLoad
{
    ...
    _shareItem = [[UIBarButtonItem alloc]initWithImage:[[ImageConstant shareImage]navShareNormal] style:UIBarButtonItemStyleBordered target:self action:@selector(share)];
    self.navigationItem.rightBarButtonItems = @[_shareItem,_favoriteItem,_likeItem];
...
- (void)tapToComments
{
    if ([self.lblCommentCount.text intValue] > 0) {
