...
- (void)viewDidLoad
{
    ...
    [self.commentShow addGestureRecognizer:tapToComments];
}
    ...
    [ImageNews imageNewsWithContentId:self.news.contentId
                             complete:^(ImageNews *news, NSError *error) {
                                 dispatch_async(dispatch_get_main_queue(), ^{
                                     ...
                                     self.lblTotalPage.text = [NSString stringWithFormat:@"/%lu",(unsigned long)news.imageList.count];
                                     self.lblCommentCount.text = [self.news.comments stringValue];
